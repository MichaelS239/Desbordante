...
std::unordered_map<std::size_t, std::size_t> CrossISNBuilder::BuildISNs() {
    ...
    for (std::size_t i = 0; i != df_packs.size(); ++i) {
        if (!df_packs[i].is_distance_ordered) {
            BuildNotDistanceOrdered(df_packs[i]);
        } else {
            BuildDistanceOrdered(df_packs[i]);
        }
    }
...
void CrossISNBuilder::SetNumMask(Cluster const& first_cluster, Cluster const& second_cluster,
    ...
    for (std::size_t i = 0; i != first_cluster.size(); ++i) {
        std::size_t const first_tuple_index = first_cluster[i];
        ...
        for (std::size_t j = 0; j != second_cluster.size(); ++j) {
            std::size_t const second_tuple_index = second_cluster[j];
