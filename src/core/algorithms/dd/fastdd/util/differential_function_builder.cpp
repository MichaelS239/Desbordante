...
std::vector<std::size_t> DifferentialFunctionBuilder::SampleRows(std::size_t row_limit) const {
    ...
    for (std::size_t i = 0; i != row_limit;) {
        ...
        if (std::find(row_nums.begin(), row_nums.end(), num) == row_nums.end()) {
...
std::pair<std::vector<double>, std::vector<double>> DifferentialFunctionBuilder::SampleThresholds(
    ...
    for (std::size_t i = 0; i != thresholds.size(); ++i) {
        frequencies.push_back(diff_freq_map[thresholds[i]]);
    }
    ...
    unique_less_thresholds.insert(unique_less_thresholds.end(), less_thresholds_set.begin(),
                                  less_thresholds_set.end());
    ...
    unique_greater_thresholds.insert(unique_greater_thresholds.end(),
                                     greater_thresholds_set.begin(), greater_thresholds_set.end());
...
void DifferentialFunctionBuilder::AddThresholds(std::vector<double> const& less_thresholds,
    ...
    for (auto threshold_it = less_thresholds.begin(); threshold_it != less_thresholds.end();
         ++threshold_it) {
    ...
    for (auto threshold_it = greater_thresholds.rbegin(); threshold_it != greater_thresholds.rend();
         ++threshold_it) {
        differential_functions_[column_index].push_back(df_provider_.GetDifferentialFunction(
