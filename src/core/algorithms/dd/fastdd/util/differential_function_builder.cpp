...
std::vector<DifferentialFunction> DifferentialFunctionBuilder::GetThresholds(
    ...
    std::ranges::transform(limits, std::back_inserter(dfs), [&dif_column](auto const& constraint) {
        return DifferentialFunction{constraint, dif_column.GetColumn()};
    });
...
void DifferentialFunctionBuilder::AddThresholds(std::vector<double> const& less_thresholds,
    ...
    if (!limits.empty()) {
        ...
        differential_functions_[column_index].reserve(limits.size());
        std::ranges::transform(limits, std::back_inserter(differential_functions_[column_index]),
                               [column](auto const& constraint) {
                                   return DifferentialFunction{constraint, column};
                               });
        dif_func_nums_.push_back(
                dif_func_nums_[dif_func_nums_.size() - 1] +
                differential_functions_[differential_functions_.size() - 1].size());
        LOG_DEBUG("Column: {}", column_index);
        for (auto const& dif_func : differential_functions_[column_index]) {
